...
public class UserController
  ...
  public void populateRentTable(ArrayList<Order> orderAl) {
    ...
    // * Order CostpDay
    pCost.setCellValueFactory(new PropertyValueFactory<Order, Double>("cost"));
  ...
  public void pSave(ActionEvent e) {
    ...
    while (userLi.hasNext()) {
      User user = (User) userLi.next();
      if (
        user.getUsername().equals(LoginController.loggedInUsername)
  ...
  public void calcDuration(ActionEvent e) throws ParseException {
    durationTxt.setText(
      String.valueOf(
        ChronoUnit.DAYS.between(startDate.getValue(), returnDate.getValue()) + 1
      )
    );
  }
  ...
  public void rentRequest(ActionEvent event) {
    ...
    ArrayList<Car> carAl = super.sortByLatestCar(
      new FileService().readCarData()
    );
    ...
    while (carLi.hasNext()) {
      ...
      if (
        (car.get_id()) ==
        (availableCarTable.getSelectionModel().getSelectedItem().get_id())
      ) {
  ...
  public void carSelected() {
    availableCarTable.setOnMousePressed(
      new EventHandler<MouseEvent>() {
        ...
        public void handle(MouseEvent event) {
          ...
          modelNum.setText(
            availableCarTable.getSelectionModel().getSelectedItem().getModel()
          );
          brandTxt.setText(
            availableCarTable.getSelectionModel().getSelectedItem().getBrand()
          );
          year.setText(
            Integer.toString(
              availableCarTable.getSelectionModel().getSelectedItem().getYear()
            )
          );
          cost.setText(
            Double.toString(
              availableCarTable.getSelectionModel().getSelectedItem().getCost()
            )
          );
        }
      }
  ...
  public void populateReturnTable(String loggedInUsername) {
    ...
    while (orderLi.hasNext()) {
      Order order = (Order) orderLi.next();
      if (
        order.getClientID() == Integer.parseInt(txtUID.getText()) &&
        !order.isPaid()
      ) {
    ...
    // * Order CostpDay
    rCost.setCellValueFactory(new PropertyValueFactory<Order, Double>("cost"));
    // * Order rent start date
  ...
  public void orderSelected() {
    carOnRentTable.setOnMousePressed(
      new EventHandler<MouseEvent>() {
        @Override
        public void handle(MouseEvent event) {
          orderLbl.setText(
            Integer.toString(
              carOnRentTable.getSelectionModel().getSelectedItem().get_id()
            )
          );
          numPlateLbl.setText(
            carOnRentTable.getSelectionModel().getSelectedItem().getPlateNum()
          );
          vehicleLbl.setText(
            carOnRentTable.getSelectionModel().getSelectedItem().getVehicle()
          );
          startDateLbl.setText(
            (
              carOnRentTable.getSelectionModel().getSelectedItem().getRentOn()
            ).toString()
          );
          returnDateLbl.setText(
            (
              carOnRentTable.getSelectionModel().getSelectedItem().getReturnOn()
            ).toString()
          );
          durationLbl.setText(
            Long.toString(
              carOnRentTable.getSelectionModel().getSelectedItem().getDuration()
            )
          );
